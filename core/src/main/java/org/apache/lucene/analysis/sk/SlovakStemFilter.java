...
/**
 * A {@link TokenFilter} that applies {@link SlovakStemmer} to stem Slovak words.
 * <p>
    ...
    public boolean incrementToken() throws IOException {
        if (input.incrementToken()) {
            if(!keywordAttr.isKeyword()) {
                final String out = stemmer.stem(termAtt.buffer(), termAtt.length());
                termAtt.copyBuffer(out.toCharArray(), 0, out.length());
            }
            return true;
        } else {
            return false;
