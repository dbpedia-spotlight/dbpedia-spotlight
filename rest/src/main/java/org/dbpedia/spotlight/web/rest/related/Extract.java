...
public class Extract {
    ...
    public Response getJSON(@DefaultValue("") @QueryParam("text") String text,
        ...
        try {
            return ok(asJSON(new Text(text), nHits));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public String asJSON(Text text, int nHits) {
        String response = OutputSerializer.tagsAsJson(text, ExtractTags.extractor().extract(text, nHits));
    ...
    @POST
    @Produces({MediaType.TEXT_XML,MediaType.APPLICATION_XML})
    public Response postXML(@DefaultValue("") @FormParam("text") String text,
                            @DefaultValue("250") @FormParam("n") int nHits) {
    ...
    public String asXML(Text text, int nHits) {
        //PrettyPrinter printer = new scala.xml.PrettyPrinter(80, 2);
        Node node = OutputSerializer.tagsAsXml(text, ExtractTags.extractor().extract(text, nHits));
