package org.dbpedia.spotlight.topic.wikipedia

import io._
import scala.collection.mutable._
import util.WikipediaApiRequest
import xml.XML
...
object TopicKeywordsFromWikipedia {
  ...
  def writeVectors(inputFile:String, outputFile:String) {
    ...
    topicVectors.foreach {
      case (topic,vector) => {
        var  vector2 = vector.map[(String,Double),List[(String,Double)]] {case (word, value) =>
        ...
        vector2 = vector2.map[(String,Double),List[(String,Double)]] {case (word, value) => (word,value/sum) }

        writer.print(topic)
        vector2.sortBy(-_._2).foreach { case (word,value) => writer.print(" "+word+":"+value)}
        writer.println()
      }
    }
    writer.close()
