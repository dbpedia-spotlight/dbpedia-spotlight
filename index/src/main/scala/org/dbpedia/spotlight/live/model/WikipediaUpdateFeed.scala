...
import org.dbpedia.spotlight.model.{Text, DBpediaCategory, DBpediaResource}

/**
 * Feed of paragraphs of wikipedia updates, which is an implementation of wikipedia harvester's external processor.
 */
class WikipediaUpdateFeed extends Feed[(DBpediaResource, Set[DBpediaResource], Set[DBpediaCategory], Text)](false) with IExternalProcessor {
  ...
  override def processAnnotations(annotationIndexMap: java.util.Map[java.lang.Integer, AnnotationIndex[Annotation]], sofaTextToName: java.util.Map[String, String]) {
    ...
    var offset = 0
    var result = List[(Text, Set[DBpediaResource])]()

    val notAllowedPattern = Pattern.compile( "Category:.*")

    text.split(WikiOccurrenceSource.splitDocumentRegex).foreach(paragraph => {
      //TODO check heuristic
      if (paragraph.size > 50 && !notAllowedPattern.matcher(paragraph).matches()) {
        offset = text.indexOf(paragraph)
        result +:=(new Text(paragraph), (Set[DBpediaResource]() ++ links.filter {
