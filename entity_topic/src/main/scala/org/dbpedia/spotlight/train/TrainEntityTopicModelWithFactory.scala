                        ...
                        currentAnnotations.
                            ...
                            spots.withFilter(spot => !currentAnnotations.exists(_.textOffset == spot.textOffset) && isSurfaceFormAllowed(spot.surfaceForm.name)).flatMap(spot => {
                                ...
                                if(total > 0) {
                                    ...
                                    Some((candidates.dropWhile(cand => {rand -= cand.support; rand < 0 }).head.resource.uri,spot.surfaceForm.name))
                                } else None
                            })
...
object EntityCollapsedGibbsSamplerHandler extends CollapsedGibbsSamplerHandler {
    def sampler(v:Iterable[Var], factors:Iterable[Factor], sampler:CollapsedGibbsSampler)(implicit random: scala.util.Random): CollapsedGibbsSamplerClosure = {
        ...
        val parentFactor = factors.collectFirst({case f:PlatedDiscreteMixture.Factor => f})
        val childMixtureFactor = factors.collectFirst({case f:PlatedCategoricalMixture.Factor => f})
        val childPlatedDiscreteFactor = factors.collectFirst({case f:PlatedDiscreteFromInducedDistribution.Factor => f})
...
object AssignmentsCollapsedGibbsSamplerHandler extends CollapsedGibbsSamplerHandler {
    def sampler(v:Iterable[Var], factors:Iterable[Factor], sampler:CollapsedGibbsSampler)(implicit random: scala.util.Random): CollapsedGibbsSamplerClosure = {
        if (v.size != 1 || factors.size != 3) return null
        val childMixtureFactor = factors.collectFirst({case f:PlatedCategoricalMixture.Factor => f})
        val parentPlatedDiscreteFactor = factors.collectFirst({case f:PlatedDiscreteFromInducedDistribution.Factor => f})
