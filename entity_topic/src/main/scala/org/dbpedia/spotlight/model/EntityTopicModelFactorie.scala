...
class EntityTopicModel(val numTopics: Int,
  ...
  object TopicSeqDomain extends DiscreteSeqDomain {
    override val elementDomain = TopicDomain
  }

  class Topics(len: Int) extends DiscreteSeqVariable(len) {
    def domain = TopicSeqDomain
  }
