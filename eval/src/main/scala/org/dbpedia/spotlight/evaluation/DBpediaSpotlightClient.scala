...
//    val support = 0;
    ...
    val directory =  LuceneManager.pickDirectory(new File(configuration.getContextIndexDirectory));
    val cache = JCSTermCache.getInstance(new LuceneManager.BufferedMerging(directory), configuration.getMaxCacheSize);
